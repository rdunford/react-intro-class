...
class Calculator extends Component {
    ...
    setDisplay(num) {
        if (this.state.resetDisplay) {
            let display = num;
            this.setState({ resetDisplay: false, display: this.state.display.length < 13 ? display : this.state.display });
        } else {
    ...
    setOperator(sign) {
        if (!this.state.operator) {
            this.setState({ operator: sign, temp: parseInt(this.state.display, 10), display: '0' })
        }
    }
    ...
    calculate() {
        ...
        this.setState({ display: String(result), operator: '', temp: 0, resetDisplay: true });
    }
